    ...
    <div class="table-top">
      ...
      <div class="table-header">
        ...
        <div class="button-wrapped">
          <el-button type="primary" plain @click="openAdduser(1)">添加产品管理员</el-button>
        </div>
      </div>
    </div>
    <!-- 表格内容 -->
...
import create_Amin from './components/create_Amin.vue'
  ...
  ElMessageBox.confirm('是否确认取消管理员资格', 'WARNING', {
    ...
    .then(async() => {
      ...
      if (res.data.status == 0) {
        ...
        await tracking('产品管理员',row.account, '高级')
        getPuser()
      } else {
...
const getListLength = async() => {
  const res = await getPuserListLength('产品管理员')
...
const getFirstPageList = async() => {
  const res = await returnListData('产品管理员', 1)
  tableData.value= res.data
}
getFirstPageList()
// 监听页面改变页数
const currentChange = async(value: number) => {
  ...
  const res = await returnListData('产品管理员',paginationData.value.currentPage)
  tableData.value = res.data
}
// 获取管理员列表，添加以后更新管理员列表
