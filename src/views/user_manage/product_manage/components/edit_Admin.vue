    ...
    <el-dialog v-model="dialogFormVisible" :title="title" width="500" height="700">
      ...
      >
        ...
        <el-form-item label="身份" prop="identify">
          <el-input v-model="ruleForm.identify" disabled />
        </el-form-item>
...
import {editPUser, getUserinfo} from '@/api/userinfo/index'
...
ruleForm.value.id=res.data[0].id
ruleForm.value.account=res.data[0].account
ruleForm.value.name=res.data[0].name
ruleForm.value.sex=res.data[0].sex
ruleForm.value.email=res.data[0].email
ruleForm.value.department=res.data[0].department
ruleForm.value.identify=res.data[0].identify
})
bus.on('titleId', (Id) => {
  if (Id == 1) {
    title.value = '编辑产品管理员'
...
const Editadmin=async()=>{
  ...
  if(res.data.status==0){
    ElMessage({
      ...
      // 确认以后，进行刷新获取列表
  emit('success')
  await tracking('Editadmin',ruleForm.value.account, '低级')
