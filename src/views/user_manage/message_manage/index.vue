    ...
    <div class="table-top">
      ...
      <div class="table-header">
        ...
        <div class="button-wrapped">
          <el-button type="primary" plain @click="openAdduser(2)">添加信息管理员</el-button>
        </div>
      </div>
    </div>
    <!-- 表格内容 -->
...
import edit_Admin from '../product_manage/components/edit_Admin.vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import { deletePuser, getPuserListLength, returnListData, searchPuser } from '@/api/userinfo'
// 面包屑参数
const item = reactive({
  first: '用户管理',
  second: '消息管理员'
...
const deletePUser = (row: any) => {
  ElMessageBox.confirm('是否确认取消管理员资格', 'WARNING', {
...
const getListLength = async() => {
  const res = await getPuserListLength('信息管理员')
...
const getFirstPageList = async() => {
  const res = await returnListData('信息管理员', 1)
  tableData.value = res.data
}
getFirstPageList()
// 监听页面改变页数
const currentChange = async(value: number) => {
  ...
  const res = await returnListData('信息管理员', paginationData.value.currentPage)
  tableData.value = res.data
}
//获取管理员列表，添加以后更新管理员列表
const getPuser = async() => {
  ...
  if (tableData.value.length == 0) {
    ...
    // 刷新页面数据
    getFirstPageList()
    paginationData.value.pageCount--
  }
}
getPuser()
</script>
