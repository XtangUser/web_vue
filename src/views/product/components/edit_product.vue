<template>
  <div>
    <!-- 弹窗 -->
    <el-dialog v-model="dialogFormVisible" title="产品信息编辑" width="500" height="700">
      <!-- 表格 -->
      <el-form
        :model="ruleForm"
        label-width="auto"
      ...
      >
        <el-form-item label="入库编号" prop="product_id">
          <el-input v-model="ruleForm.product_id" disabled />
        </el-form-item>
...
// // 产品入库-创建产品
import { editProduct } from '@/api/product/product'
// 调用方法
...
bus.on('editAuitProductId', (row:any) => {
  ruleForm.id=row.id
